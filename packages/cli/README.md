...
`asset init --from-audit` is a limited Phase 1 alternative that accepts only a
complete successful audit report. It is a mutating direct CLI authoring action:
review one selected finding and explicitly consent to leave the read-only audit
before running it. The read-only audit Skill never runs it.
...
This alternative does not create a strict authoring session or its receipts,
drawing contract, contract-bound candidate import, or session recovery state.
...
#### Strict animation-remediation journey

First save the complete report for one bounded audit scope; do not reduce it to
a paginated discovery result or discard its `unsupported`, `missingFiles`,
`blankFrames`, or `errors` categories.

```sh
lpc-toolkit catalog audit-animations --animation climb --json > audit.json
```

Prepare `plan.json` using schema `lpc-toolkit.asset-authoring-plan.v1`. It is
explicit input based on one human-selected finding, the complete report digest,
and human-provided draft credits; the CLI does not select a finding or generate
the plan from the report automatically. The installed npm package includes the
[complete `examples/extend-item-plan.v1.json` template](examples/extend-item-plan.v1.json).
It is a syntactically valid post-consent field guide, not audit or attribution
evidence. Copy it only after reviewing and confirming one finding, replace
every example value, and never infer report evidence, consent, authorship, or
license. Starting the completed plan creates the session. Use the returned
session ID for every later session command.
...
Inspect the drawing contract before an external artist or provider-neutral tool
creates candidate pixels. Import binds the candidate to the exact contract
digest; validation must then succeed before the attributed review-ready preview
publishes matching metadata, TXT, and CSV credits. This is the default stopping
boundary. Use `asset authoring status` and `resume` for digest-bound receipts and
recovery rather than bypassing stale or interrupted evidence. See the
[provider-neutral handoff](#optional-provider-neutral-agent-handoff) and
[install, registry, and recovery policy](#install-registry-and-recovery-policy)
for detailed receipts and recovery behavior.

Warning acknowledgement, declaration, preview acceptance, sync, formal pack,
inspection, provenance, and installation are a separately confirmed formal
lifecycle; none follows automatically from review-ready output. After a
successful exact installation, rerun `catalog audit-animations` with the same
bounded audit scope and inspect every finding category. Exit code zero only
means the audit ran; closure requires the selected finding to disappear or an
explicit residual disposition while no new scoped regressions appear.
...
path confidence. The published package carries a complete, parser-verified
`examples/extend-item-plan.v1.json`; the Codex plugin carries a byte-identical
copy. Neither copy selects a finding or supplies consent or attribution
authority. Attach-pack sessions can inspect an existing pack, but this
...
and CSV credits. The animation starter directly selects
`$lpc-animation-asset-audit` and does not preselect `$lpc-asset-authoring`;
after confirmation, the authoring Skill uses its packaged byte-identical copy
of `extend-item-plan.v1.json` rather than guessing the strict plan structure.
