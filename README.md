...
existing character, `asset init --new` creates a new source item, the strict
session below remediates one selected audit finding, and formal pack/install
commands handle separately authorized release and consumer activation.

#### Strict animation-remediation session

First save a bounded read-only animation audit. `plan.json` is explicit input
prepared from one selected finding and draft credits supplied by a human; it is
not generated from the report automatically. The published npm package ships a
[complete `extend-item` plan template](packages/cli/examples/extend-item-plan.v1.json)
at `examples/extend-item-plan.v1.json`. Copy it only after the finding and scope
are confirmed, then replace every example value with the complete report
evidence and human-provided attribution. The Codex plugin carries the same
versioned template for its authoring Skill.
...
The CLI validates contract-bound candidate pixels supplied by an external
artist or tool; it does not draw them. Stop at the attributed review-ready
preview unless the user separately requests release or installation. See the
[CLI asset lifecycle guide](packages/cli/README.md#strict-asset-authoring-sessions)
for the plan schema, provider-neutral options, receipts, recovery, and formal
lifecycle details.

#### Limited Phase 1 scaffold alternative

`asset init --from-audit` remains a limited Phase 1 alternative for a complete,
...
This is a mutating direct CLI authoring action. Before running it, review one
selected finding and explicitly consent to leave the audit; the read-only audit
Skill never runs it. It cannot scaffold `blankFrames`, audit `errors` never
become drawing tasks, and it does not create a strict authoring session or its
receipts. Use `asset init --new` for a brand-new source item.
...
A journey kickoff prompt only starts the installed Agent integration. The
animation launcher and plugin starter directly select
`$lpc-animation-asset-audit`, which conducts the read-only audit and shows the
bounded finding; they do not preselect the mutating Skill. After explicit
confirmation, `$lpc-asset-authoring` continues in the same task through a
strict authoring session using the versioned plan template: plan, drawing
contract, candidate import, validation, and attributed preview. If either Skill
is unavailable, use the public CLI directly with the same authority boundaries.
