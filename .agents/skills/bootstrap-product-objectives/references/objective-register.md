...
Create stable, reviewable identifiers for every auditable statement in the current Product Direction while preserving the English document as the canonical normative source and keeping both Product Direction files free of registry metadata. Keep the register English-only.
...
Store the complete register at `docs/PRODUCT-OBJECTIVES.md`. Product Direction owns product semantics; the register owns objective identity, classification, and English source mapping. Current capability specs map only IDs declared in this register.
...
The text after `—` is a concise English objective, not a second normative contract. The English source locator contains an exact Markdown heading, ` > `, and an exact prose excerpt that uniquely identifies the corresponding sentence or clause within that heading. Keep the excerpt as short as possible while remaining unique. Do not use line numbers or descriptive labels that are absent from Product Direction. Do not add translated source locators or translated objective text.
